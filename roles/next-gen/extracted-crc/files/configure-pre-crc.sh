...
CLOUD_INIT_KEYS_URL=${CLOUD_INIT_KEYS_URL:-'http://169.254.169.254/1.0/meta-data/public-keys'}
...
if [ -n "$CLOUD_INIT_KEYS" ]; then
    ...
    do
        ...
        curl -LO "$CLOUD_INIT_KEYS/$AVAILABLE_KEY/openssh-key"

        if ssh-keygen -l -f openssh-key; then
            cat openssh-key | tee -a "${USER_DIR}/.ssh/authorized_keys"
        fi
    done
fi
